...
- 학생 Today와 컨디션 체크
- 공부 타이머
- 하나의 할 일 데이터를 오늘·금주·캘린더로 연결한 통합 일정 보드
- 공부·운동·회복 할 일 추가·삭제·완료와 모바일 주간·월간 전환
- 확인형 일정 도우미
- 학생 선택형 학부모 공개 권한
- 학부모 문의 메시지
- 강사 우선 확인 화면
...
| `/calendar` | 오늘·금주 할 일 관리, 확인형 일정 도우미, 주간/월간 앱 캘린더 |
| `/guardian` | 학부모 코치 문의 |
...
일정 화면의 공부·운동·회복 할 일은 날짜가 있는 하나의 데이터로 관리합니다. 같은 항목이 오늘 보드, 향후 7일 보드, 캘린더 선택 날짜에 동기화되며 학생 화면에서만 추가·삭제·완료 상태를 변경할 수 있습니다. 현재는 브라우저 로컬 저장 프로토타입이므로 실제 운영 전 서버 권한 검사와 PostgreSQL 저장이 필요합니다.
