...
- 홈의 자연어 요청을 일정 자동 등록 또는 코치 대화로 연결하는 RP AI 도우미
...
현재 일정 도우미는 비용 없는 규칙 기반 해석기로 날짜, 시간, 제목과 분류를 정리합니다. 홈에서 보낸 요청은 날짜와 시간이 모두 확인된 경우 자동 저장하고 캘린더로 이동하며, 정보가 부족한 경우 초안만 보여줍니다. 캘린더에서 직접 입력한 요청은 사용자가 초안을 확인한 뒤 저장합니다. 향후 OpenAI API로 교체하더라도 로그인·승인·일일 사용량 제한을 먼저 적용합니다.
