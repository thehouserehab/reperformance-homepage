...
- 문서 버전: 1.0
- 작성일: 2026-08-02
- 최근 반영: 학생 홈 3단계 집중 구조·Notion식 사이드바·목적형 RP AI 연결
...
## 1.0 학생 홈 집중 구조와 사이드바 업데이트

학생 첫 화면은 `오늘 상태 저장`, `오늘의 할 일`, `RP AI 도우미` 세 영역만 남겼습니다. 공부·운동·회복 과제는 홈에서 즉시 체크해 완료 처리할 수 있지만, 추가·삭제와 전체 계획 조정은 캘린더의 선택 날짜 영역에서 수행합니다. 공부 타이머는 기능을 제거하지 않고 `/student/study` 전용 화면으로 분리했습니다.

홈과 일정 페이지의 역할도 분리했습니다. 홈은 오늘 바로 해야 할 행동, 캘린더는 미래 계획과 날짜별 편집에 집중합니다. 일정 페이지에 반복되던 대형 오늘·금주 보드는 제거했고, 캘린더 안에서 선택한 날짜의 할 일과 일정을 추가·완료·삭제하도록 정리했습니다. 코치 화면은 같은 내용을 읽기 전용으로 유지합니다.

RP AI 도우미는 문장을 분석해 날짜·시간이 명확한 일정 요청은 자동 등록 후 캘린더로 이동하고, 입시·운동·식단·멘탈 등 상담 요청은 AI 문장 정리 모드가 열린 담당 코치 대화창으로 전달합니다. 원문은 URL에 넣지 않고 같은 브라우저의 임시 세션에만 전달합니다. 밤 10시 이후의 격려는 알림 권한을 요구하지 않는 작은 홈 상단 문구로 시작하며, 푸시 알림은 운영 인프라와 사용자 동의가 준비된 뒤 검토합니다.

하단 고정 내비게이션은 제거하고 목록 아이콘을 누르면 열리는 Notion식 사이드바로 교체했습니다. 학생에게는 오늘·캘린더·공부 타이머·기록·상담·코치 대화·공개 설정을 제공하며, 다른 역할은 각자 필요한 목록만 봅니다. 1440x900 PC와 390x844 모바일에서 홈 세 영역, 사이드바, 할 일 체크, 일정 자동 등록·삭제, 코치 상담 전달, 코치 일정 읽기 전용, 가로 넘침과 오류 오버레이를 검증했습니다.

## 0.9 통합 일정 보드 업데이트

노션 `To-do 캘린더`의 핵심인 **하나의 할 일 데이터를 여러 시점에서 다시 보여주는 구조**를 RP APP에 적용했습니다. 공부·운동·회복 할 일에 날짜를 부여하고, 같은 데이터를 `오늘의 할 일`, `금주의 할 일`, 캘린더 선택 날짜에서 동기화해 보여줍니다. 학생은 오늘과 향후 7일 보드에서 항목을 추가·삭제·완료할 수 있고, 코치는 같은 흐름을 읽기 전용으로 확인합니다.

데스크톱에서는 오늘과 금주 보드를 나란히 배치해 학업과 실기 부담을 비교하기 쉽게 만들었습니다. 모바일에서는 `오늘 → 금주 → 일정 도우미 → 캘린더` 순서로 단순화하고, 캘린더는 주간 보기를 기본으로 사용합니다. 선택 날짜의 공부·운동·회복 과제와 일반 일정은 달력 바로 아래에 함께 표시되어 날짜를 선택한 뒤 다른 화면으로 이동할 필요가 없습니다.

일정 도우미는 큰 독립 패널이 아니라 오늘·금주 계획을 확인한 다음 캘린더에 반영하는 중간 단계로 배치했습니다. AI 또는 규칙 기반 해석 결과는 여전히 초안으로만 제공하며 학생이 확인하기 전에는 저장하지 않습니다. 1440x900 데스크톱과 390x844 모바일에서 추가·삭제·완료, 주간 기본 보기, 선택 날짜 동기화, 코치 읽기 전용 권한, 가로 넘침과 오류 오버레이를 검증했습니다.

## 0.8 모바일 일정 탐색과 웜 팔레트 보정

모바일 일정 화면은 `오늘의 할 일 → 일정 도우미 → 달력` 순서로 재배치했습니다. 학생은 달력 전체를 먼저 해석하지 않고 지금 해야 할 일을 확인할 수 있으며, 자연어 일정 등록이 필요한 경우 바로 아래의 일정 도우미를 사용할 수 있습니다. 달력은 모바일에서 주간 보기가 기본이고 주간·월간 분할 선택으로 전환할 수 있으며, 데스크톱에서는 정보 밀도를 고려해 월간 보기를 기본으로 유지합니다.

기존 포레스트 그린에서 차갑게 느껴질 수 있는 청록 기를 줄이고 올리브 기가 섞인 세이지 그린, 웜 아이보리, 절제된 살구 오렌지로 전역 토큰을 보정했습니다. 브라우저 테마색과 PWA 배경색도 같은 기준으로 맞췄습니다. 390px 모바일에서 섹션 순서, 7일 주간 보기, 42일 월간 보기, 할 일 완료 처리, 일정 초안 생성·확인 등록을 실제 클릭으로 검증했으며 가로 넘침과 오류 오버레이가 없음을 확인했습니다.

## 0.7 브랜드 색감 업데이트

기존의 청록과 푸른 회색 중심 팔레트를 포레스트 그린, 딥 그린, 따뜻한 오프화이트와 살구 오렌지 조합으로 조정했습니다. 학생 홈, 캘린더, 학생-코치 대화, 입력창, 하단 내비게이션 등 전역 토큰과 주요 하드코딩 색상을 함께 변경해 화면별 색감이 어긋나지 않도록 했습니다.

브랜드의 초록색 정체성은 유지하되 배경과 중간 회색에 따뜻한 기운을 더했습니다. 핵심 텍스트·버튼 조합은 WCAG 일반 텍스트 기준인 4.5:1 이상의 명암 대비를 유지하며, 390px 모바일 세 화면에서 가로 넘침과 오류 오버레이가 없음을 확인했습니다.

## 0.6 대화 화면 디자인 업데이트

학생과 코치의 대화 화면을 일반 정보 페이지 안의 입력 폼에서 대화 전용 화면으로 전환했습니다. 큰 페이지 제목을 시각적으로 제거하고, 상대방 중심 헤더, 날짜 구분선, 발신자별 비대칭 말풍선, 말풍선 밖 시간, 한 줄 자동 확장 작성창을 적용했습니다.

직접 입력과 AI 문장 정리 선택, 사용자 확인 후 전송, 학부모 비공개 원칙은 그대로 유지합니다. 390px 모바일에서는 페이지 자체의 가로·세로 스크롤 없이 대화와 작성창, 역할별 하단 내비게이션이 한 화면에 들어오며, 학생·코치 양쪽 화면과 AI 초안 전송 흐름을 브라우저에서 검증했습니다.

## 0.5 DB·인증 권한 설계 업데이트

2차 ERD를 계정·관계, 학생 기록, 대화·AI·동의·감사, RLS의 네 개 전방향 PostgreSQL 마이그레이션으로 구체화했습니다. 25개 핵심 테이블은 학생 본인, 활성 담당 코치, 학생이 확인한 학부모 연결과 항목별 공개 설정, 실제 대화 참여자를 DB에서 다시 검사합니다.

비밀번호와 인증 토큰은 저장하지 않고 외부 인증의 `auth_subject`만 연결합니다. 메시지는 평문 컬럼이 아닌 암호문 바이트를 사용하고, AI 요청 감사에는 프롬프트·응답·성적·건강 원문을 저장하지 않습니다. AI 사용은 기능별 승인과 일일 잔여량을 원자적으로 예약합니다.

현재 SQL은 설계와 정적 검증을 완료했지만 PostgreSQL 엔진에는 적용하지 않았습니다. 다음 단계는 인증 제공자와 미성년자 확인 정책을 확정한 뒤 격리된 테스트 DB에서 전체 마이그레이션과 RLS를 실제 실행하는 것입니다.

## 0.4 2차 구조 업데이트

학생·코치·학부모·관리자의 역할과 접근 경계를 매트릭스로 확정하고, 역할별 주 메뉴를 학생 5개·코치 4개·학부모 2개·관리자 4개로 제한했습니다. 학생 기록·상담, 코치 기록 검토, 학부모 공개 요약, 관리자 운영·관계·AI·감사 화면의 뼈대를 앱에 반영했습니다.

PostgreSQL을 붙이기 전에 계정·활성 관계·학부모 공개 설정·공부·실기·컨디션·일정·대화·AI 승인·동의·감사 모델을 ERD로 정리했습니다. 이 단계는 운영 기능을 완성한 것이 아니라 이후 인증과 DB가 따라야 할 구조적 기준을 확정한 것입니다.
...
학생과 코치가 각 역할 화면에서 같은 대화를 확인하고 답장하는 흐름을 구현했습니다. 직접 작성이 기본이며, AI 도움을 선택하면 메모를 문장 초안으로 정리한 뒤 사용자가 확인하고 직접 전송합니다. 현재 초안 정리는 외부 모델과 토큰을 사용하지 않는 로컬 규칙 기반 기능입니다.

학부모 화면에는 학생-코치 대화를 노출하지 않습니다. 다만 현재 구현은 한 브라우저의 `localStorage`를 공유하는 기능 시연용 프로토타입이므로, 실제 서로 다른 계정과 기기에서 사용하려면 인증·역할 관계 권한·PostgreSQL 저장·실시간 동기화·읽음 상태·알림이 추가되어야 합니다.
...
학생용 내비게이션은 목록 아이콘으로 여는 사이드바에 모읍니다. 첫 화면에서 이동 선택지를 계속 노출하지 않고, 필요할 때만 열어 디지털 피로도를 줄입니다.

| 내비게이션 | 핵심 질문 | 포함 기능 |
| --- | --- | --- |
| 오늘 | 지금 무엇을 해야 하는가? | 상태 저장, 공부·운동·회복 완료, RP AI 도우미 |
| 캘린더 | 앞으로 무엇을 조정해야 하는가? | 주간·월간 일정, 선택 날짜 할 일 편집 |
| 공부 타이머 | 지금 공부를 어떻게 시작하는가? | 과목, 목표, 집중·휴식, 세션 기록 |
| 기록 | 무엇을 해왔는가? | 운동, 실기, 공부, 식단, 컨디션 |
| 상담 | 지금 무엇을 물어봐야 하는가? | 입시, 운동, 식단, 멘탈 상담 허브 |
| 코치 대화 | 담당 코치와 무엇을 조율하는가? | 직접 메시지, 선택형 AI 문장 정리 |
| 공개 설정 | 부모님에게 무엇을 공개하는가? | 출결, 계약·결제, 학업, 실기 선택 공개 |
...
RP APP 캘린더는 학생이 확인하는 공부·운동·상담·시험 일정의 앱 내 기준입니다. 홈 RP AI 도우미는 날짜와 시간이 모두 확인되는 요청만 자동 저장하고, 정보가 부족하면 초안으로 남깁니다. 캘린더에서 직접 작성한 요청은 학생이 확인한 뒤 저장합니다. 성적·건강·상담 원문은 URL이나 외부 캘린더 이벤트에 포함하지 않습니다.
...
4. 학생 홈은 `오늘 상태·오늘 할 일·RP AI 도우미` 세 영역으로 제한하고, 나머지 기능은 접이식 사이드바에서 선택합니다.
