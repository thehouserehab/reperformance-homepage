...
- `/api/rp/clients` returns a bounded customer list with pagination metadata; the default page should stay at 200 rows and the request maximum at 500 rows unless a load test proves a larger value is safe. PostgreSQL pages use `nextCursor` keyset pagination and `rp_clients_page_cursor_idx`; `nextOffset` remains fallback compatibility only. The staff customer manager should use the 더 불러오기 flow instead of one-shot full-table downloads.
