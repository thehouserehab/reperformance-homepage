...
export function getAiDailyLimitMax() {
  return parsePositiveInteger(process.env.RP_AI_DAILY_LIMIT_MAX, 50);
}

function capAiDailyLimit(value, fallback = 1) {
  return Math.min(parsePositiveInteger(value, fallback), getAiDailyLimitMax());
}

export function getAiDailyLimitForRole(role, account = null) {
  if (Number(account?.aiDailyLimit) > 0) return capAiDailyLimit(account.aiDailyLimit);

  const normalizedRole = normalizeRole(role);
  const defaultLimit = parsePositiveInteger(process.env.RP_AI_DAILY_LIMIT, 3);

  if (hasStaffRole(normalizedRole)) {
    return capAiDailyLimit(
      process.env.RP_AI_STAFF_DAILY_LIMIT || process.env.RP_AI_DAILY_LIMIT,
      20,
    );
  }

  return capAiDailyLimit(process.env.RP_AI_MEMBER_DAILY_LIMIT || process.env.RP_AI_DAILY_LIMIT, defaultLimit);
}
