...
import { getPostgresConnectionOptions } from './rpPostgresSsl.js';
import {
  buildDefaultConsultationSlots,
...
export function getDatabasePoolConfig(databaseUrl = getDatabaseUrl()) {
  ...
  return {
    ...
    sslEnabled: databaseUrl ? connectionOptions.ssl !== false : null,
  };
}
...
export async function ensureDatabaseSchema() {
  ...
  schemaPromise = (async () => {
    ...
    await pool.query('CREATE INDEX IF NOT EXISTS rp_clients_page_cursor_idx ON rp_clients (updated_at DESC, created_at DESC, id DESC)');
...
export async function listDatabaseClients({ limit = 200, offset = 0, cursor = null } = {}) {
  if (!isDatabaseConfigured()) return { clients: [], cursorRows: [] };
  ...
  const result = cursor
    ? await queryDatabase(
      `SELECT *
       FROM rp_clients
       WHERE (updated_at, created_at, id) < ($2::timestamptz, $3::timestamptz, $4::text)
       ORDER BY updated_at DESC, created_at DESC, id DESC
       LIMIT $1`,
      [safeLimit, cursor.updatedAt, cursor.createdAt, cursor.id],
    )
    : await queryDatabase(
      `SELECT *
       FROM rp_clients
  ...
  return {
    clients: result.rows.map(mapClientRow).filter(Boolean),
    cursorRows: result.rows.map((row) => ({
      id: String(row.id || ''),
...
let defaultConsultationSlotSyncPromise = null;
let defaultConsultationSlotSyncedAt = 0;

async function ensureDefaultDatabaseConsultationSlots({ now = new Date() } = {}) {
  if (!isDatabaseConfigured()) return;

  const cacheWindowMs = 15 * 60 * 1000;
  if (Date.now() - defaultConsultationSlotSyncedAt < cacheWindowMs) return;
  if (defaultConsultationSlotSyncPromise) return defaultConsultationSlotSyncPromise;

  const slots = buildDefaultConsultationSlots({ now });
  if (!slots.length) return;

  defaultConsultationSlotSyncPromise = (async () => {
    const values = [];
    const placeholders = slots.map((slot, index) => {
      const offset = index * 4;
      values.push(slot.id, slot.startsAt, slot.endsAt, 'system-default-hours');
      return `($${offset + 1}, $${offset + 2}, $${offset + 3}, TRUE, $${offset + 4}, NOW())`;
    });
    ...
    defaultConsultationSlotSyncedAt = Date.now();
  })();

  try {
    await defaultConsultationSlotSyncPromise;
  } finally {
    defaultConsultationSlotSyncPromise = null;
  }
}

export async function listPublicDatabaseConsultationSlots({ now = new Date(), limit = 1000 } = {}) {
  if (!isDatabaseConfigured()) return [];
  await ensureDefaultDatabaseConsultationSlots({ now });
  const range = getConsultationAvailabilityRange(now);
  const safeLimit = clampInteger(limit, 1000, 1, 2400);
  const result = await queryDatabase(
    `SELECT s.id, s.starts_at, s.ends_at, s.is_open
     ...
     WHERE s.is_open = TRUE
       AND s.starts_at >= $1
       AND s.starts_at <= $2
       AND s.ends_at - s.starts_at = INTERVAL '30 minutes'
       AND EXTRACT(ISODOW FROM s.starts_at AT TIME ZONE 'Asia/Seoul') BETWEEN 1 AND 5
       AND (s.starts_at AT TIME ZONE 'Asia/Seoul')::time >= TIME '09:00'
       AND (s.ends_at AT TIME ZONE 'Asia/Seoul')::time <= TIME '22:00'
       AND NOT EXISTS (
         SELECT 1
         FROM rp_clients c
