...
function getSslConfig(databaseUrl) {
  if (cleanEnvValue(process.env.RP_DATABASE_SSL).toLowerCase() === 'false') return false;
...
function mapClientRow(row) {
  ...
  return {
    ...
    parqYesItems: Array.isArray(row.parq_yes_items) ? row.parq_yes_items : [],
    goal: String(row.goal || ''),
    purpose: Array.isArray(row.purpose) ? row.purpose : [],
    painAreas: Array.isArray(row.pain_areas) ? row.pain_areas : [],
