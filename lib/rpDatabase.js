...
export async function checkDatabaseSchemaReadiness() {
  ...
  try {
    ...
    const tableResult = await pool.query(
      ...
      ['public.rp_auth_accounts'],
    );
    const authAccountsTableExists = Boolean(tableResult.rows[0]?.table_name);
    ...
    const missingRequiredIndexes = verifiedContactUniqueIndexPresent
      ? []
      : [AUTH_VERIFIED_CONTACT_UNIQUE_INDEX];
