...
export function isSharedRateLimitFailClosed() {
  return ['1', 'true', 'yes', 'on', 'enabled'].includes(normalizeFlag(process.env.RP_RATE_LIMIT_FAIL_CLOSED));
}

export function getSharedRateLimitFailureRetrySeconds() {
  return Math.min(
    300,
    Math.max(5, parsePositiveInteger(process.env.RP_RATE_LIMIT_FAIL_CLOSED_RETRY_SECONDS, 60)),
  );
}
...
async function checkSharedRateLimit(key, limit, windowMs) {
  try {
    ...
    if (isSharedRateLimitFailClosed()) return getSharedRateLimitFailureRetrySeconds();
