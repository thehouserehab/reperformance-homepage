...
function getAppsScriptSecret() {
  return cleanValue(
    process.env.RP_APPLICATION_NOTIFICATION_SECRET
...
export async function sendApplicationNotification(application = {}) {
  ...
  const response = await fetchWithTimeout(
    webhookUrl,
    {
      ...
      body,
      cache: 'no-store',
      // Apps Script ContentService returns a one-time googleusercontent.com URL.
      // Google requires HTTP clients to follow that redirect to read the JSON result.
      redirect: provider === 'google-apps-script' ? 'follow' : 'error',
    },
    {
