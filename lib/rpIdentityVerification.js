...
function getVerificationSecret() {
  return assertStrongProductionSecret(
    ...
    'RP_IDENTITY_VERIFICATION_SECRET or fallback signing secret',
  );
}
...
export async function requestIdentityCode(payload = {}) {
  ...
  if (!contact) throw exposePublicError(`${getIdentityMethodLabel(method)} 정보를 입력해주세요.`);
  if (method === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact)) {
    throw exposePublicError('올바른 이메일 주소를 입력해주세요.');
  }
  if (method === 'phone' && contact.length < 9) {
    throw exposePublicError('올바른 전화번호를 입력해주세요.');
  }
