...
async function sign(value) {
  ...
  if (!secret) throw new Error('RP_IDENTITY_VERIFICATION_SECRET 또는 RP_ADMIN_SESSION_SECRET 환경변수가 필요합니다.');
...
export function normalizeIdentityMethod(value) {
  const method = cleanValue(value).toLowerCase();
...
async function sendIdentityCode({ method, contact, code, purpose }) {
  ...
  const message = `[RePERFORMANCE] ${purpose === 'signup' ? '회원가입' : '계정 찾기'} ${label} 인증번호는 ${code}입니다. ${Math.floor(CODE_TTL_SECONDS / 60)}분 안에 입력해주세요.`;

  if (!webhookUrl) {
    return {
      sent: false,
      setupRequired: true,
      message: `${label} 인증 웹훅이 설정되지 않았습니다.`,
    };
  }
