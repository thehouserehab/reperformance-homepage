...
async function callAuthScript(action, payload = {}, options = {}) {
  ...
  const response = await fetch(buildScriptUrl(action), fetchOptions);
...
export async function findSheetAuthAccount(username, password) {
  ...
  if (account.passwordHash) {
    ...
    if (attemptedHash && account.passwordHash === attemptedHash) return sanitizeAccount(account);
  }

  if (account.password && account.password === normalizedPassword) return sanitizeAccount(account);

  return null;
}

export async function findSheetAuthAccountByIdentity(name, phone) {
  const normalizedName = cleanEnvValue(name).toLowerCase();
  const normalizedPhone = normalizePhone(phone);
  if (!normalizedName || !normalizedPhone || !getAuthWebAppUrl()) return null;
