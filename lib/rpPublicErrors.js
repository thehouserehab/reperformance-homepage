...
export function getSafeLogErrorDetail(error, fallback = 'internal_error') {
  return {
    name: cleanMessage(error?.name || 'Error').slice(0, 80) || 'Error',
    status: getPublicErrorStatus(error),
    message: getSafePublicErrorMessage(error, fallback),
