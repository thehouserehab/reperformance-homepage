import { findDatabaseAuthAccount, isDatabaseOnlyMode } from './rpDatabase';
...
export async function findAuthAccountFromStores(username, password) {
  ...
  const databaseAccount = await findDatabaseAuthAccount(username, password).catch((error) => {
    if (isDatabaseOnlyMode()) throw error;
    return null;
  });
  if (databaseAccount) return databaseAccount;
  if (isDatabaseOnlyMode()) return null;
