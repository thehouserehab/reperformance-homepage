...
function normalizeIdentityMethod(value) {
  const method = cleanEnvValue(value).toLowerCase().replace(/[\s_-]+/g, '');
...
function normalizeAuthSource(value) {
  const source = cleanEnvValue(value).toLowerCase();
  if (['database', 'sheet', 'env'].includes(source)) return source;
  return '';
}

export function getAdminSessionTtlSeconds() {
  const seconds = parsePositiveInteger(process.env.RP_SESSION_TTL_SECONDS || process.env.RP_ADMIN_SESSION_TTL_SECONDS);
  if (seconds) return Math.max(seconds, MIN_SESSION_TTL_SECONDS);

  const days = parsePositiveInteger(process.env.RP_SESSION_TTL_DAYS || process.env.RP_ADMIN_SESSION_TTL_DAYS);
  if (days) return Math.max(days * 24 * 60 * 60, MIN_SESSION_TTL_SECONDS);
...
function getAdminSecret() {
  return assertStrongProductionSecret(
    process.env.RP_ADMIN_SESSION_SECRET || process.env.RP_API_SECRET,
    'RP_ADMIN_SESSION_SECRET or RP_API_SECRET',
  );
}
...
function isEnabledFlag(value) {
  return ['1', 'true', 'yes', 'on'].includes(cleanEnvValue(value).toLowerCase().replace(/[\s_-]+/g, ''));
}

export function areEnvironmentAuthAccountsAllowed() {
  if (process.env.NODE_ENV !== 'production') return true;
  return isEnabledFlag(process.env.RP_ALLOW_ENV_AUTH_ACCOUNTS);
}
...
function toAccount(username, password, extra = {}, defaultRole = 'member') {
  ...
  return {
    ...
    phone: cleanEnvValue(extra.phone || extra.tel || extra.mobile || extra['연락처'] || extra['전화번호']),
    email: cleanEnvValue(extra.email || (normalizedUsername.includes('@') ? normalizedUsername : '')),
    kakaoId: cleanEnvValue(extra.kakaoId || extra.kakao || extra.kakaoTalkId),
    role,
  };
}
...
export function findAuthAccount(username, password) {
  ...
  return getAuthAccounts().find((account) => (
    account.username.toLowerCase() === normalizedUsername && safeEqual(account.password, normalizedPassword)
  )) || null;
}

export function findAuthAccountByIdentity(name, contact, method = 'phone') {
  ...
  return getAuthAccounts().find((account) => (
    cleanEnvValue(account.name).toLowerCase() === normalizedName &&
    normalizeIdentityContact(
      normalizedMethod,
      normalizedMethod === 'email'
        ? account.email || account.username
        : normalizedMethod === 'kakao'
          ? account.kakaoId || account.kakao
          : account.phone,
    ) === normalizedContact
  )) || null;
}
...
export async function createAdminSession(account) {
  ...
  const payload = {
    ...
    authSource: normalizeAuthSource(account.authSource || account.source),
    sessionVersion: normalizeSessionVersion(account.sessionVersion || account.session_version),
    iat: now,
    exp: now + ttlSeconds,
  };
...
export async function verifyAdminSessionCookie(cookieValue) {
  ...
  try {
    ...
    return {
      ...
      authSource: normalizeAuthSource(payload.authSource),
...
function getAdminCookieSecurityOptions() {
  return {
    httpOnly: true,
