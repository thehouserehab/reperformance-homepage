...
export async function findAuthAccountByIdentityFromStores(name, phone) {
  const envAccount = findAuthAccountByIdentity(name, phone);
  if (envAccount) return { account: envAccount, source: 'env', passwordResetSupported: false };

  const database = await import('./rpDatabase');
  const databaseOnly = database.isDatabaseOnlyMode();
  const databaseAccount = await database.findDatabaseAuthAccountByIdentity(name, phone).catch((error) => {
    if (databaseOnly) throw error;
    return null;
  });
  ...
  const sheetAccount = await findSheetAuthAccountByIdentity(name, phone);
